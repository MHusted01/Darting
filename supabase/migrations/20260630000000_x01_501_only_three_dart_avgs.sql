-- Phase 12b: headline 3-dart averages are 501-only. The x01 slug covers both
-- 501 and 301 (config->>'startingScore'); a 501 average is the number players
-- compare to pub/league play, so every cloud 3DA aggregate now also filters
-- COALESCE(config->>'startingScore', '501') = '501'. Missing config defaults
-- to 501 (legacy sessions). games_played and win_rate remain all-games.
  ...
  LEFT JOIN LATERAL (
    SELECT
      ...
      AVG(gp.three_dart_avg) FILTER (
        WHERE gp.three_dart_avg IS NOT NULL
          AND gs.game_slug = 'x01'
          AND COALESCE(gs.config->>'startingScore', '501') = '501'
    ...
    FROM (
      SELECT
        ...
        AVG(gp2.three_dart_avg) FILTER (
          WHERE gp2.three_dart_avg IS NOT NULL
            AND gs2.game_slug = 'x01'
            AND COALESCE(gs2.config->>'startingScore', '501') = '501'
        ) AS slug_avg
...
-- while avg_three_dart_avg is 501-only (the headline X01 metric). A member who
-- only plays Cricket or 301 shows games_played > 0 with a null average — intended.
