-- Fix get_player_public_stats to include sessions where the user is a
-- participant but not the creator (multi-player games). The gp.user_id = u.id
-- join already identifies the user's own row; the extra created_by = u.id
-- constraint incorrectly excluded those sessions.
