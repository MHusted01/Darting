...
interface FullBoardInputProps {
  onDartThrown: (dart: DartThrow) => void;
  isProcessing: boolean;
}

export function FullBoardInput({ onDartThrown, isProcessing }: FullBoardInputProps) {
    ...
    return (
      <View className="gap-3">
        ...
        <View className="flex-row gap-2">
          ...
          >
            <Text className="text-base font-barlow-semi text-ds-on-surface">Single</Text>
          ...
          {!isBull && (
            ...
            >
              <Text className="text-base font-barlow-semi text-ds-on-surface">Triple</Text>
              <Text className="text-xs font-barlow text-ds-on-surface-variant mt-0.5">
                {pendingSegment * 3}
              </Text>
            </Pressable>
    ...
    <View className="gap-2">
        ...
        <View key={rowIdx} className="flex-row gap-2">
          {row.map((segment) => (
            <Pressable
              ...
              disabled={isProcessing}
              className={`flex-1 h-11 bg-ds-surface border border-ds-outline-variant rounded-xl items-center justify-center active:opacity-70${isProcessing ? ' opacity-50' : ''}`}
              accessibilityRole="button"
              accessibilityLabel={`Segment ${segment}`}
            >
              <Text className="text-sm font-barlow-semi text-ds-on-surface">{segment}</Text>
            </Pressable>
          ))}
        </View>
      ))}
      <View className="flex-row gap-2 mt-1">
        ...
        <Pressable
          onPress={handleMiss}
          disabled={isProcessing}
          className={`flex-1 h-11 bg-ds-surface-low border border-ds-outline-variant rounded-xl items-center justify-center active:opacity-70${isProcessing ? ' opacity-50' : ''}`}
          accessibilityRole="button"
          accessibilityLabel="Miss"
        >
          <Text className="text-sm font-barlow-semi text-ds-on-surface-variant">Miss</Text>
        </Pressable>
      </View>
    </View>
  );
