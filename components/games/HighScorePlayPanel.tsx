...
interface HighScorePlayPanelProps {
...
export function HighScorePlayPanel({
  players,
  currentPlayerId,
  turnDarts,
  isProcessing,
  onDartThrown,
}: HighScorePlayPanelProps) {
  const currentPlayer = players.find((p) => p.id === currentPlayerId);
  if (!currentPlayer) return null;

  const state = currentPlayer.gameState as HighScorePlayerState;
    ...
    <View className="gap-6">
      ...
      <View className="bg-ds-surface border border-ds-outline-variant rounded-xl overflow-hidden">
        {players.map((player, idx) => {
          const ps = player.gameState as HighScorePlayerState;
