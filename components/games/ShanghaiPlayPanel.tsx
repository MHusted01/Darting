...
export function ShanghaiPlayPanel({
  players,
  currentPlayerId,
  turnDarts,
  isProcessing,
  onDartThrown,
}: ShanghaiPlayPanelProps) {
  const currentPlayer = players.find((p) => p.id === currentPlayerId);
  if (!currentPlayer) return null;

  const state = currentPlayer.gameState as ShanghaiPlayerState;
    ...
    <View className="gap-6">
      <View className="items-center">
        <Text className="text-xs font-barlow-semi text-ds-on-surface-variant uppercase tracking-widest mb-1">
          Round {round} of {SHANGHAI_MAX_ROUNDS}
        </Text>
        ...
        <Text className="text-sm font-barlow text-ds-on-surface-variant mt-1">
          Target: <Text className="font-barlow-semi text-ds-on-surface">{round}</Text>
        </Text>
      </View>
      ...
      <View className="bg-ds-surface border border-ds-outline-variant rounded-xl overflow-hidden">
        {players.map((player, idx) => {
          const ps = player.gameState as ShanghaiPlayerState;
